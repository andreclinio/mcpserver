...
void ev_handler(struct mg_connection *nc, int ev, void *ev_data)
{
    ...
    server->log("Received path: " + uri + " :: " + path_prefix);
    ...
    server->log("Path as request: " + request_type);
...
MacRouteInfo MacServer::findRoute(MacRequestType requestType, std::string uri)
{
    ...
    for (it = routes.begin(); it != routes.end(); ++it)
    {
        ...
        {
            auto map = utils_match_paths(route_path, uri_path);
            if (!map.empty())
            {
                result.route = r;
                result.path_map = map;
...
bool MacServer::handleEvent(struct mg_connection *nc, struct http_message *hm, MacRequestType requestType, std::string uri)
{
    ...
    auto route = route_info.route;
    if (route == NULL)
    {
        return false;
    }
    auto request_id = utils_generate_uuid_v4();
